...
#define kPRAnimationDuration .20f
...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        stateLabel.backgroundColor = kPRBGColor;
        stateLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        stateLabel.text =  NSLocalizedString(@"下拉刷新", @"");
        [self addSubview:stateLabel];
        _stateLabel = stateLabel;
        
        UILabel *dateLabel = [[UILabel alloc] init ];
        dateLabel.font = ft;
        dateLabel.textColor = kTextColor;
        dateLabel.textAlignment = NSTextAlignmentCenter;
        dateLabel.backgroundColor = kPRBGColor;
...
- (void)changeLanguage
{
    [_headerView changeLanguage];
    [_footerView changeLanguage];
}
